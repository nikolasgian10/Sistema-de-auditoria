    ...
    <div className="flex h-screen overflow-hidden">
      ...
      >
        <div className="flex h-16 items-center gap-3 border-b border-sidebar-border px-6">
          ...
          <div>
            <h1 className="text-lg font-bold text-sidebar-primary-foreground">Auditoria</h1>
            <p className="text-[10px] uppercase tracking-widest text-sidebar-foreground/60">Mahle</p>
          </div>
        </div>
          ...
          <div className="border-t border-sidebar-border px-4 py-3 space-y-2">
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                {(() => {
                  console.log('AppLayout renderizando minifabrica:', { 
                    minifabrica: currentUser.minifabrica, 
                    nome: currentUser.minifabrica ? getMinifabricaName(currentUser.minifabrica) : 'Sem minifábrica', 
                    fullUser: currentUser 
                  });
                  return (
                    <p className="text-[10px] text-sidebar-foreground/50">
                      {currentUser.minifabrica ? getMinifabricaName(currentUser.minifabrica) : 'Sem minifábrica'}
                    </p>
                  );
                })()}
              </div>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <header className="flex h-16 shrink-0 items-center gap-4 border-b bg-card px-4 lg:px-6 no-print">
          ...
          {/* Show locked minifábrica for Diretor */}
          {userType === 'diretor' && currentUser?.minifabrica && (() => {
            console.log('AppLayout renderizando minifabrica para Diretor:', currentUser.minifabrica);
            return (
              <div className="flex items-center gap-2">
                <Factory className="h-4 w-4 text-muted-foreground" />
                <Badge variant="secondary" className="text-xs">
                  {getMinifabricaName(currentUser.minifabrica)}
                </Badge>
              </div>
            );
          })()}
