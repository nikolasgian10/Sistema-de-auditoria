...
import { useChecklists, useAddChecklist, useUpdateChecklist } from '@/hooks/use-checklists';
...
export default function ChecklistTemplate() {
  ...
  useEffect(() => {
    if (editingId && checklists.length > 0) {
  ...
  }, [editingId, checklists]);

  const updateItem = (id: string, field: keyof LPAItem, value: any) => {
  ...
  const loadFromChecklist = (ckId: string) => {
  ...
  const handleSaveChecklist = async () => {
    ...
    if (editingId || selectedChecklist) {
      ...
      updateChecklist.mutate({ id: idToUpdate, ...checklistData }, {
        onSuccess: () => navigate('/checklists'),
      });
    } else {
      addChecklist.mutate(checklistData, {
        onSuccess: () => navigate('/checklists'),
      });
    }
  };
  ...
  const handlePrint = () => {
    ...
    win.document.write(`<html><head><title>${title}</title><style>
      *{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;font-size:12px}@page{size:A4 landscape;margin:8mm}
      .page{page-break-after:always;border:2px solid #222;position:relative}.page:last-child{page-break-after:auto}
      .side{position:absolute;right:0;top:0;bottom:0;width:36px;background:${sideColor};writing-mode:vertical-rl;text-orientation:mixed;display:flex;align-items:center;justify-content:center;color:white;font-weight:bold;font-size:13px;letter-spacing:2px}
      .title-bar{background:${sideColor};color:white;padding:8px 50px 8px 12px;font-size:16px;font-weight:bold;text-align:center}
      .hdr{padding:5px 50px 5px 12px;font-size:11px;border-bottom:1px solid #ccc}
      .item-row{display:grid;border-bottom:1px solid #bbb;min-height:80px}.item-area{border-right:1px solid #bbb;writing-mode:vertical-rl;text-orientation:mixed;display:flex;align-items:center;justify-content:center;font-weight:bold;font-size:12px;background:#f5f5f5;padding:4px}
      .item-q{padding:8px;border-right:1px solid #bbb;display:flex;align-items:center}.item-e{padding:8px;color:#555;display:flex;align-items:center;font-size:11px}
      ...
      .turno-active{background:#222;color:white}.status-col{border-right:1px solid #bbb;display:flex;align-items:center;justify-content:center;padding:4px}
    </style></head><body>`);
    ...
    win.document.write(`<div style="display:grid;background:#e8e8e8;font-weight:bold;font-size:10px;border-bottom:2px solid #222;grid-template-columns:70px 1fr 1fr;padding-right:36px"><div style="padding:5px;border-right:1px solid #bbb;text-align:center">ITEM</div><div style="padding:5px;border-right:1px solid #bbb;text-align:center">PERGUNTAS</div><div style="padding:5px;text-align:center">EXPLICAÇÃO</div></div>`);
    items.forEach(item => {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3 no-print">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Button size="sm" variant="default" onClick={handleSaveChecklist}>
            <Save className="mr-1 h-3 w-3" />Salvar Checklist
          </Button>
      ...
      <div className="border-2 border-foreground/30 rounded-sm overflow-hidden relative max-w-5xl mx-auto bg-white text-black">
        <div className="absolute right-0 top-0 bottom-0 w-9 flex items-center justify-center font-bold text-xs text-white z-10"
          ...
          <input className="bg-transparent text-white text-center font-bold text-xs w-full border-none outline-none"
            style={{ writingMode: 'vertical-rl' }} value={sideLabel} onChange={e => setSideLabel(e.target.value)} />
        </div>
      ...
      <div className="border-2 border-foreground/30 rounded-sm overflow-hidden relative max-w-5xl mx-auto bg-white text-black mt-6">
        ...
        <div className="pr-11 px-3 py-1 text-xs border-b border-black/20 flex gap-6">
          <span><strong>NOME AUDITOR:</strong> {auditorName || '___________________'}</span>
          <span><strong>EP MÁQUINA:</strong> ___________________</span>
        </div>
            ...
            <div className="border-r border-black/20 p-2 text-[10px] space-y-1">
              ...
              <label className="flex items-center gap-1 cursor-pointer">
                <input type="checkbox" checked={item.actionImmediate}
                  onChange={e => updateItem(item.id, 'actionImmediate', e.target.checked)} className="w-3 h-3" />
              ...
              <label className="flex items-center gap-1 cursor-pointer">
                <input type="checkbox" checked={item.escalate}
                  onChange={e => updateItem(item.id, 'escalate', e.target.checked)} className="w-3 h-3" />
                <span>via SFM</span>
              </label>
            </div>
