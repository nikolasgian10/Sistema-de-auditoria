...
import { useMachines, useAddMachine, useUpdateMachine, useDeleteMachine, Machine } from '@/hooks/use-machines';
import { useChecklists } from '@/hooks/use-checklists';
import { useAuth } from '@/lib/auth';
import { MINIFABRICAS } from '@/lib/constants';
...
function exportToCSV(machines: Machine[]) {
  ...
  const rows = machines.map(m => [m.name, m.code, m.sector, m.description, m.created_at]);
...
export default function Machines() {
  ...
  const handleSave = () => {
    ...
    if (editing) {
      updateMachine.mutate({ id: editing.id, ...form });
    } else {
      addMachine.mutate(form);
    }
    setFormOpen(false); setEditing(null); resetForm();
  };

  const handleDelete = (id: string) => deleteMachine.mutate(id);

  const handleEdit = (m: Machine) => {
    setEditing(m);
    setForm({ name: m.name, code: m.code, sector: m.sector, minifabrica: m.minifabrica || '', description: m.description });
    setFormOpen(true);
  };
  ...
  const handleImportCSV = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (ev) => {
      ...
      const rows: { name: string; code: string; sector: string; minifabrica: string; description: string }[] = [];
      lines.slice(1).forEach(line => {
        const cols = line.split(',').map(c => c.replace(/^"|"$/g, '').trim());
        if (cols[0] && cols[1]) {
          rows.push({ name: cols[0], code: cols[1], sector: cols[2] || '', minifabrica: cols[3] || '', description: cols[4] || '' });
        }
      });
      if (rows.length > 0) {
        ...
        toast.success(`${rows.length} máquina(s) importada(s)`);
      }
      setImportOpen(false);
    };
  ...
  const printLabels = useCallback(async (machs: Machine[]) => {
    ...
    try {
      ...
      for (let i = 0; i < machs.length; i++) {
        ...
        label.style.cssText = `width:660px;height:210px;display:flex;align-items:center;gap:18px;padding:20px 24px;border:2px solid #9e9e9e;border-radius:10px;box-sizing:border-box;background:linear-gradient(to right,#f4f4f4,#e6e6e6);font-family:Arial, Helvetica, sans-serif;`;
        label.innerHTML = `
          ...
          <div style="flex:1;display:flex;align-items:center;justify-content:center;text-align:center;font-size:24px;font-weight:700;color:#1f1f1f;line-height:1.3;word-break:break-word;">${labelText}</div>
          <div style="flex-shrink:0;width:96px;height:96px;display:flex;align-items:center;justify-content:center;">${qrSvg}</div>
        `;
        stage.appendChild(label);
        await new Promise(requestAnimationFrame);
        const canvas = await html2canvas(label, { scale: 2, useCORS: true, logging: false, backgroundColor: '#ffffff' });
        const imgData = canvas.toDataURL('image/png');
        if (x + labelWidthMm > pageWidth - margin + 0.01) { x = margin; y += labelHeightMm + gapY; }
        if (y + labelHeightMm > pageHeight - margin + 0.01) { pdf.addPage(); x = margin; y = margin; }
  ...
  if (isLoading) return <div className="flex items-center justify-center py-12"><p className="text-muted-foreground">Carregando máquinas...</p></div>;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Cadastro e gestão de máquinas · {machines.length} registros</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Dialog open={importOpen} onOpenChange={setImportOpen}>
            ...
            <DialogContent>
              ...
              <p className="text-sm text-muted-foreground mb-2">Formato: Nome, Código, Setor, Minifábrica, Descrição (1ª linha = cabeçalho)</p>
            ...
            <DialogContent>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Select value={form.minifabrica} onValueChange={v => setForm(f => ({ ...f, minifabrica: v }))}>
